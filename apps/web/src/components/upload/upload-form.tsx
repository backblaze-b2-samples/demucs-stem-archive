...
import { ingestTrack } from "@/lib/api-client";
    ...
    for (const rejection of rejections) {
      ...
      const errors = rejection.errors.map((e) => {
        if (e.code === "file-too-large") {
          return `exceeds 200MB limit (${humanizeBytes(rejection.file.size)})`;
        }
        if (e.code === "file-invalid-type") {
          return "must be MP3, WAV, or FLAC";
        }
        return e.message;
      });
  ...
  const handleFilesSelected = useCallback((files: File[]) => {
    ...
    const uploadQueue = async () => {
      ...
      for (const item of newItems) {
        try {
          await ingestTrack(item.file, (percent) => {
            setItems((prev) =>
              prev.map((i) =>
          ...
          toast.success(`${item.file.name} archived — separating stems…`);
          anySuccess = true;
        } catch (err) {
          const message =
          ...
          toast.error(`Failed to ingest ${item.file.name}: ${message}`);
        }
      }
      setUploading(false);
      // Refresh so the Library + dashboard pick up the new processing track.
  ...
  return (
    <Card>
      <CardHeader className="border-b border-border py-4 px-5">
        <CardTitle className="card-title">Add Tracks</CardTitle>
      </CardHeader>
