...
def download_file(key: str) -> bytes:
    """Fetch an object's bytes from B2 via get_object.

    Used by the re-separate path, which pulls an already-archived original
    back out of the bucket to run separation again. Raises FileNotFoundError
    when the key is missing, RuntimeError on any other S3 failure.
    """
