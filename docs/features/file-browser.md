...
## Purpose
List, preview, download, and delete every object in the bucket. This is the
generic full-bucket explorer kept from the starter kit — it sees all keys,
including the `tracks/` archive. The **Stem Library** (`/library`) is the
app-specific, scoped view; it reuses this router's `/preview` and `/download`
endpoints for stem streaming and download.
...
- Preview: opens dialog, fetches a preview-only presigned URL via `/files/{key}/preview` (does not count as a download) and streams the audio inline
